...
generate: bin/mga bin/entc ## Generate code
	go generate -x ./...
	mga generate kit endpoint ./internal/app/chat_app/messaging/...
	mga generate event handler --output subpkg:suffix=gen ./internal/app/chat_app/messaging/...
	mga generate event dispatcher --output subpkg:suffix=gen ./internal/app/chat_app/messaging/...
	entc generate ./internal/app/chat_app/messaging/messagingadapter/ent/schema
...
proto: bin/protoc bin/protoc-gen-go bin/protoc-gen-kit buf ## Generate client and server stubs from the protobuf definition
	...
	./bin/protoc-${PROTOC_VERSION}/bin/protoc -I bin/protoc-${PROTOC_VERSION} -I api/proto --go_out=plugins=grpc,import_path=$(shell go list .):.gen/api/proto --kit_out=.gen/api/proto $(shell find api/proto -name '*.proto')
